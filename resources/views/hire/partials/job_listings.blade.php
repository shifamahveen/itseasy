<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    @foreach($jobs as $job)
        <div class="border rounded-lg p-4 shadow-lg border-l-4 border-gray-200">
            <div class="flex justify-between space-x-4">
                <div class="flex space-x-4">
                    <img src="{{ asset('uploads/logos/' . $job->logo) }}" alt="{{ $job->company }}" class="h-16 w-16 mb-4">
                    <h2 class="text-xl font-semibold">{{ $job->title }}</h2>
                </div>
                
                <form method="POST" action="{{ route('job.bookmark', $job->id) }}" class="ms-auto">
                    @csrf
                    <button type="submit">
                        <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M21 11.0975V16.0909C21 19.1875 21 20.7358 20.2659 21.4123C19.9158 21.735 19.4739 21.9377 19.0031 21.9915C18.016 22.1045 16.8633 21.0849 14.5578 19.0458C13.5388 18.1445 13.0292 17.6938 12.4397 17.5751C12.1494 17.5166 11.8506 17.5166 11.5603 17.5751C10.9708 17.6938 10.4612 18.1445 9.44216 19.0458C7.13673 21.0849 5.98402 22.1045 4.99692 21.9915C4.52615 21.9377 4.08421 21.735 3.73411 21.4123C3 20.7358 3 19.1875 3 16.0909V11.0975C3 6.80891 3 4.6646 4.31802 3.3323C5.63604 2 7.75736 2 12 2C16.2426 2 18.364 2 19.682 3.3323C21 4.6646 21 6.80891 21 11.0975ZM8.25 6C8.25 5.58579 8.58579 5.25 9 5.25H15C15.4142 5.25 15.75 5.58579 15.75 6C15.75 6.41421 15.4142 6.75 15 6.75H9C8.58579 6.75 8.25 6.41421 8.25 6Z" fill="#f43f5e"></path>
                        </svg>
                    </button>
                </form>
            </div>
            
            <p class="text-gray-700 flex space-x-2">
                <span>{{ $job->company }}</span> 
            </p>
            <p class="text-gray-500 flex space-x-2">
                <span>{{ $job->locations }}</span>    
            </p>
            <div class="mt-4 flex gap-4 justify-end">
                <form method="GET" action="{{ route('job.applyForm', $job->id) }}">
                    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-400">
                        Apply
                    </button>
                </form>
            </div>
        </div>
    @endforeach
</div>
